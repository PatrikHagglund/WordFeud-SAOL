...
# ÉÈÀ men tex ej ÅÄÖ
accent_map = str.maketrans({"É": "E", "È": "E", "À": "A"})
...
def filtrera_ord_saol(saol_fil, saol_csv_fil):
    ...
    filtrerade_ord = [w for w in alla_ord if w.lower() not in namn_ord]
...
def rensa_tecken(indata_ord):
    ...
    print(
        f"Antal ord kvar efter rensning och filtrering på tecken: {len(godkanda_ord)}"
    )
    return godkanda_ord
